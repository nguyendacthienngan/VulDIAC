...
node_labels = ["Block", "Call", "Comment", "ControlStructure", "File", "Identifier", "Field_Identifier", "Literal",
               "Local", "Member", "MetaData", "Method", "MethodInst", "MethodParameterIn", "MethodParameterOut",
               "MethodReturn", "Namespace", "NamespaceBlock", "PARAM", "Return", "Type", "TypeDecl", "Unknown"]
...
class Node:
    ...
    def set_properties(self, node, attrs):
        ...
        if 'LINE_NUMBER' in attrs:
            self.line_number = attrs['LINE_NUMBER']
        else:
    ...
    #     self.type = self.line_info.split(",")[0]
    #     # self.code = self.line_info.split(",")[1]
    #     # 代码截取第一个逗号后的部分
    #     # group the node types
    #     self.code = self.line_info[self.line_info.index(",") + 1:]
    #     # if ("<operator>" in self.type) or ("<operators>" in self.type):
    #     #     self.type = "<operator>" + self.type.split(".")[1]
        
    #     if self.type in keywords or 'fun' in self.type:
    #         self.type = "CALL"
    ...
    # def _set_label(self):
    #     self.label = node_labels.get(self.type)
    #     if self.label is None:
    #         self.label = node_labels.get("Unknown")
            
    def __str__(self):
        return f"Node {self.id} ({self.type}): {self.code}"
